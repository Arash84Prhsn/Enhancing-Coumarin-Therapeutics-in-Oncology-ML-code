...
def dose_ranges_for_target(coumarin, time_point, target=50, tol=2):
    """
    Returns a list of continuous dose ranges where |viability - target| <= tol.
    Each range is a tuple: (dose_start, dose_end)
    """
    ...
    # check if last interval goes to the end
    if in_range:
        ranges.append((start, doses[-1]))
...
for coumarin in coumarins:
    for t in time_points:
        vrange = viability_range(coumarin, t)
        dranges = dose_ranges_for_target(coumarin, t)
        ...
        final_results.append({
            ...
            "Coumarin": coumarin,
            "Time (h)": t,
            "Viability Min": round(vrange[0], 2),
            "Viability Max": round(vrange[1], 2),
            "Dose Ranges for Viability 50±2": formatted
        })

df_final = pd.DataFrame(final_results)
df_final.drop(columns=['Viability Min', 'Viability Max'],inplace=True);
print(df_final)
