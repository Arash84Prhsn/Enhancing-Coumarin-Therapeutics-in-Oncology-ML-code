...
CURRENT_MODEL = "HistGradientBoosting(AURAPTENE)";
CURRENT_COUMARIN = "Auraptene"
SEPARATE_COUMARINS = True
...
separateCoumarinDataDict = {"Auraptene" : None, "Esculetin" : None, 
                            "Galbanic Acid" : None, "Umbelliprenin" : None};
...
X = separateCoumarinDataDict[CURRENT_COUMARIN][['Cancer Type', 'Coumarin Dose', 'Time']].copy(); # Order must be ['Cancer Type', 'Coumarin Type', 'Coumarin Dose', 'Time'](Coumarin type if only doing general training)
y = separateCoumarinDataDict[CURRENT_COUMARIN]['Viability'].copy(); # Viability column
...
if DO_GRIDSEARCH :
    ...
    gs_Estimator = HistGradientBoostingRegressor();
...
def predict_viability(model, cancer_type, dose, time, cancer_encoder=None):
    """
    Predict viability for a single input or a list of inputs (separate coumarin model).
    ...
    # Build DataFrame with the same columns as training
    df = pd.DataFrame({
...
predicted_viability = predict_viability(
    ...
    dose=80,                   # dose value
    time=24,                   # time value
    cancer_encoder=Encoders[CURRENT_COUMARIN]
)
