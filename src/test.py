...
seperateCoumarinDataDict = {"Auraptene" : None, "Esculetin" : None, 
...
for coumarin in coumarins :
    ...
    seperateCoumarinDataDict[coumarin] = reliable_data;
...
X = None; # Order must be ['Cancer Type', 'Coumarin Type', 'Coumarin Dose', 'Time'](Coumarin type if only doing general training)
y = None; # Viability column
...
params = sanitize_params(estimator=None, params=params);
...
df = pd.DataFrame(cv_Scores, index=[0]).T
...
if SEPARATE_COUMARINS == False :
    ...
    # Get cancer types
    cancer_types = coumarin_data[CURRENT_COUMARIN]['Cancer Type'].unique()
