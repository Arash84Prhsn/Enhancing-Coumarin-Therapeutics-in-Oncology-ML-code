...
X['log_Dose'] = np.log10(X['Coumarin Dose'] + 1)
...
def predict_viability(model, cancer_type, coumarin_type, dose, time, 
    ...
    df['log_Dose'] = np.log10(df['Coumarin Dose'] + 1)
...
def find_ic50(doses, preds, target=50):
    """
    Find IC50 by linear interpolation assuming monotonic decreasing viability.
    """
    for i in range(len(doses) - 1):
        if preds[i] >= target and preds[i + 1] <= target:
            d0, d1 = doses[i], doses[i + 1]
            v0, v1 = preds[i], preds[i + 1]
            return d0 + (target - v0) * (d1 - d0) / (v1 - v0)
    return None


def dose_range_for_target(
    coumarin,
    time_point,
    target=50,
    tol=2,
    ...
    "Time": time_point,
    "log_Dose": np.log10(float(d) + 1)
    } for d in doses])

    preds = model.predict(X)

    # --- Step 1: Find IC50 crossing ---
    ic50 = find_ic50(doses, preds, target)
    if ic50 is None:
        return None

    idx = np.argmin(np.abs(doses - ic50))

    # --- Step 2: Expand locally within tolerance ---
    left = idx
    while left > 0 and abs(preds[left] - target) <= tol:
        left -= 1

    right = idx
    while right < len(doses) - 1 and abs(preds[right] - target) <= tol:
        right += 1

    return float(doses[left + 1]), float(doses[right - 1])
...
# Build final table with continuous dose ranges
final_results = []

for coumarin in coumarins:
    for t in time_points:
        drange = dose_range_for_target(coumarin, t)
        formatted = format_range(drange)

        final_results.append({
            "Cancer Type": "Colon",
            "Coumarin": coumarin,
            "Time (h)": t,
            "Dose Range for Viability 50±2 (%)": formatted
        })
...
df_final.to_csv(f"{RESULT_DIR}OptimizationWindows/{CURRENT_MODEL}_BestWindows.csv", index=False)
